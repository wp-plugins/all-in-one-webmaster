=== Plugin Name ===
Contributors: arpitshah
Tags: all in one webmaster, google webmaster, yahoo webmaster, bing webmaster, google analytics, arpitshah, meta tag generator, header meta, meta, footer tag, header, footer, yahoo site explorer
Requires at least: 2.6	
Tested up to: 2.8.6
Stable tag: 0.9.5

Sitemap submission to Google, Bing, Yahoo, Ask and option to add Google, Bing, Yahoo Webmaster Meta Tag to your blog and also add Google Analytics.

== Description ==
If you are a webmaster you probably heard of Google Webmaster Tool. Both Yahoo and Bing have their own versions called "Yahoo SiteExplorer" and "Bing Webmaster Central". All in One Webmaster allow you to add Google, Bing, Yahoo Webmaster Meta Tag into your blog theme and also you can add Google Analytics using just an Account ID. It uses the wp_head() and wp_footer() hooks built in to WordPress to add content to your header and/or footer. 

Sitemap Submission option to Google, Yahoo, Bing and Ask.com. There are more to come in next releases.

**Features**

* Ability to add Google, Bing, Yahoo Webmaster Meta-Tag
* Ability to add Google Analytics code
* NEW: XML Sitemap submission to Google.com, Yahoo.com and Ask.com (alpha release, report your bugs)
* NEW: Warning/Success/Failure messages added

**My other plugins**

* Twitter Goodies
* WP Archive-Sitemap Generator

== Installation ==
1. Unpack the `download-package`.
2. Upload the file to the `/wp-content/plugins/` directory.
3. Activate the plugin through the `Plugins` menu in WordPress.
4. Configure the options under Admin Panel `Settings -> All in one Webmaster`.
5. Done and Ready.

== Frequently Asked Questions ==

= Found Bug? =
Please report all you bug on Plugin Homepage.

= Where to find Google, Bing, Yahoo webmaster META tag? =
Visit links provided in `Settings -> All in One Webmaster` to get your META tag. This is an easiest way to authenticate your sites.

= Analytics Account ID =
Find the Account ID, starting with UA- in your Google Analytics's Account overview page.

== Screenshots ==
1. All in one Webmaster Admin page
2. Warning message
3. Success notification
4. Failure message

== Change Log ==

= 0.9.5 = 
* NEW: XML Sitemap submission to Google.com, Yahoo.com and Ask.com (alpha release, report your bugs)
* NEW: Validation and Warning message status
* NEW: Warning/Success/Failure messages added
* Some minor UI changes

= 0.9.2 =
* Changed Google Analytics option. (Now user has to add only ID field)

= 0.9.1 =
* Initial release
* Option to add Google, Bing, Yahoo Webmaster META tag
* Option to add Google Analytics code into blog
* More options into next releases
